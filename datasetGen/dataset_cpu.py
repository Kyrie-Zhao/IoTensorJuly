import torchvision
from testing_model_cpu import inference
import time
import os
import numpy as np
import torch
...
def file_Process(filename):
    ...
    perf_counter['CPU_Util'] = psutil.cpu_percent(1)
    return perf_counter


def profiler(pid_list,iter_model):
    model_bag = {"1":"inception_v3","2":"googlenet","3":"squeezenet","4":"shufflenet","5":"vgg_11","6":"vgg_13","7":"vgg_16","8":"mobilenet_v3_small","9":"alexnet","10":"mvit","11":"mnasnet","12":"resnext50_32x4d","13":"mobilenet_v3_large","14":"mobilenet_v2"}
...
def main():
    ...
    model_gen_total_list = model_gen_total_list[44:]
    #print(model_gen_total_list)
    #sys.exit(1)
    #model_gen_total_list = [(1,)]
    for iter_model in model_gen_total_list:
        ...
        for model_id in iter_model:
            ...
            python_cmd = ['python', 'bg_models_cpu.py', '--name', model_name, '--bs', '1']
        ...
        for _ in range(1,15):
            ...
            perf_record['inference-model-sys-info'] = {"CPU_Util":cpu_util}
            ...
            filename = "dataset_cpu/"+str(data_trace_count)+"-"+str(iter_model)+"-"+str(_)+".json"
